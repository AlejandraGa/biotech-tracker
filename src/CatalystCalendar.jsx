...
// Event type styles — NO emojis/icons
const EVENT_COLORS = {
  'PDUFA':           { color: '#1d4ed8', bg: 'rgba(29,78,216,0.08)',   border: 'rgba(29,78,216,0.25)',   label: 'PDUFA' },
  'Phase 2 Readout': { color: '#7c3aed', bg: 'rgba(124,58,237,0.08)',  border: 'rgba(124,58,237,0.25)',  label: 'Ph2 Readout' },
  'Phase 2 Interim': { color: '#6d28d9', bg: 'rgba(109,40,217,0.08)',  border: 'rgba(109,40,217,0.25)',  label: 'Ph2 Interim' },
  'Phase 3 Readout': { color: '#be123c', bg: 'rgba(190,18,60,0.08)',   border: 'rgba(190,18,60,0.25)',   label: 'Ph3 Readout' },
  'IND Expansion':   { color: '#0369a1', bg: 'rgba(3,105,161,0.08)',   border: 'rgba(3,105,161,0.25)',   label: 'IND' },
  'Conference':      { color: '#047857', bg: 'rgba(4,120,87,0.08)',    border: 'rgba(4,120,87,0.25)',    label: 'Conference' },
  'Earnings':        { color: '#a16207', bg: 'rgba(161,98,7,0.08)',    border: 'rgba(161,98,7,0.25)',    label: 'Earnings' },
  'Insider Buying':  { color: '#15803d', bg: 'rgba(21,128,61,0.08)',   border: 'rgba(21,128,61,0.25)',   label: 'Insider Buy' },
  'Dilution Risk':   { color: '#c8102e', bg: 'rgba(200,16,46,0.08)',   border: 'rgba(200,16,46,0.25)',   label: 'Dilution' },
};

function getEventStyle(type) {
  return EVENT_COLORS[type] || { color: '#555', bg: 'rgba(100,100,100,0.07)', border: 'rgba(100,100,100,0.2)', label: type };
}
...
function Spinner() {
  return <span style={{ display: 'inline-block', width: 10, height: 10, border: '1.5px solid #555', borderTopColor: '#c8102e', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />;
}
...
function CatalystCard({ event, onAIAnalysis, aiData, loadingAI }) {
  ...
  const sentimentArrow = event.sentiment === 'bullish' ? '↑' : event.sentiment === 'bearish' ? '↓' : '→';
  ...
  return (
    ...
    >
      <div style={{ padding: '14px 16px', cursor: 'pointer' }} onClick={() => setExpanded(x => !x)}>
        ...
        <div className="catalyst-card-inner" style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>

          {/* Probability ring — hidden on very small screens via CSS */}
          <div style={{ flexShrink: 0 }} className="prob-ring">
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e0d8', borderRadius: 6, padding: '14px 16px', marginBottom: '1.25rem' }}>
      ...
      <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, minmax(44px, 1fr))', gap: 4, minWidth: 480 }}>
          {weeks.map((week, i) => {
...
Cover: (1) what this trial is really testing scientifically, (2) the bull case if successful, (3) the bear case / key risks, (4) what to watch for in the data. Be specific, no disclaimers, write for a sophisticated investor.`;
  ...
  const hasActiveFilters = typeFilter || watchlistOnly || selectedWeek !== null;
    ...
    <div>
      <style>{`
        @media (max-width: 680px) {
          ...
          .cal-sidebar-left { display: none !important; }
          .cal-sidebar-right { display: none !important; }
          .cal-mobile-filter-btn { display: flex !important; }
          .cal-mobile-filter-panel { display: block !important; }
          .desktop-scores { display: none !important; }
          .mobile-scores { display: flex !important; }
          .prob-ring { display: none !important; }
        }
        @media (min-width: 681px) {
          .cal-mobile-filter-btn { display: none !important; }
          .cal-mobile-filter-panel { display: none !important; }
        }
      `}</style>
      ...
      {filtersOpen && (
        <div className="cal-mobile-filter-panel" style={{ display: 'none', background: '#fff', border: '1px solid #e5e0d8', borderRadius: 6, padding: '14px 16px', marginBottom: 16 }}>
          <FilterPanel />
        </div>
      )}

      {/* 3-col layout — collapses to 1 on mobile */}
      <div className="cal-layout" style={{ display: 'grid', gridTemplateColumns: '200px 1fr 220px', gap: '1.5rem', alignItems: 'flex-start' }}>

        {/* Left filters — desktop only */}
        <div className="cal-sidebar-left" style={{ position: 'sticky', top: '1rem' }}>
          <FilterPanel />
        </div>
